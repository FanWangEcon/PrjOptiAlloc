...
There are only two individuals, pick any feasible point, and compare against optimal allocations, at one aggregate resource point

```{r}
df_queue_il_long_with_V <- df_queue_il_long_n2
df_input_ib_agg_grp <- df_input_ib_all %>% filter(w_agg==6)

unique(df_input_ib_agg_grp %>% pull(id_alloc))


df_input_ib <- df_input_ib_all %>% filter(id_alloc==36)
df_input_ib
