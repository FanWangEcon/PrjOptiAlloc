...
description: |
  Compare welfare between all feasible discrete allocations and observed allocations when N=2.
...
First, test at one aggregate resources level:
...
Second, compute REV all all feasible allocation combinations, using [(MxP by N) to (MxQ by N+Z-1)](https://fanwangecon.github.io/R4Econ/function/dof/htmlpdfr/fs_funceval_groupwider.html):
...
Third review results: 

```{r}
# group by rho, group by w, sort by id_1l and id_2l
view(df_rev_allrho)
```
