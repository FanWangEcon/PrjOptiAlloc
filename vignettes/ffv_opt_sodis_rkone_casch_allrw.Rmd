---
title: DISCRETE--Discrete Optimal Allocation California Teacher Student Ratio (Line by Line)
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{DISCRETE--Discrete Optimal Allocation California Teacher Student Ratio (Line by Line)}
  %\VignetteEngine{knitr::rmarkdown}
  \usepackage[utf8]{inputenc}
urlcolor: blue
---

# Discrete Optimal Allocation Allocate Teacher (Line by Line)
...
ffi_disc_value_star_q <- function(fl_rho, df_queue_il,
  ...
  df_rev_Ail_onerho <- df_rev_Ail_onerho %>%
    select(!!sym(svr_inpalc), !!sym(svr_v_cumu_il_rksrt_outter))
...
ffi_solve_queue_alloc <- function(ar_rho,
    ...
    # Step 5, value at Q points
    # Evaluate REV
    ar_util_rev_loop <- df_alloc_i_long %>%
      group_by(!!sym(svr_rho)) %>%
      do(rev = ffi_disc_rev_onerho(fl_rho = .$rho_val,
    ...
    ffi_disc_value_star_q(fl_rho, df_queue_il,
                                  svr_id_i = 'id_i',
                                  svr_D_il = 'D_il', svr_inpalc = 'Q_il', svr_D_Wbin_il = 'D_Wbin_il',
                                  svr_A_il = 'A_il', svr_alpha_il = 'alpha_il', svr_beta_i = 'beta_i',
                                  svr_v_cumu_l = 'v_sum_l',
    ...
    # Sort columns
    df_alloc_i_long <- df_alloc_i_long %>%
      select(!!sym(svr_id_i), !!sym(svr_rho), !!sym(svr_rho_val),
...
df_rev_il_long_onerho_wrong <- df_rev_il_long_onerho_wrong %>%
                        mutate(!!sym(svr_v_cumu_il_rksrt_outter) :=
                                 (!!sym(svr_v_cumu_il_rksrt))^(1/fl_rho))
```
...
# if rho < 0, at rank = 0, adding up components of inner sum, v_sum_il_rk at first increasing adding through all rank = 0
# but after reach rank = 1, v_sum_il_rk starts decreasing, because each incremental allocation with rho<0 power has lower
...
# Solve for REV
start_time_nest <- Sys.time()
ar_util_rev_loop_func <- ffi_disc_rev(ar_rho, fl_teacher_increase_number, df_input_ib, df_queue_il_long)
...
sum(ar_util_rev_loop - ar_util_rev_loop_func$rev)
```
