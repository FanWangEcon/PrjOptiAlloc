...
2. If $Q_{il}\left(\lambda\right) \le \widehat{W}$, set $Z_{il}\left(\lambda\right) = 1$, otherwise $0$
...
df_all_rho_long_Ail <- df_all_rho_long %>%
  ...
  mutate(Z_il = case_when(rank <= fl_teacher_increase_number ~ 1, TRUE ~ 0)) %>%
  # select(IDil, rho, Z_il) %>%
  left_join(df_all_rho %>% select(IDil, id, Dmax_i), by='IDil')

# Generate D_i and F_i long
df_all_rho_long_D_i <- df_all_rho_long_Ail %>%
  select(id, rho, Dmax_i, Z_il) %>%
  group_by(id, rho) %>%
  summarize(D_i = sum(Z_il), Dmax_i = mean(Dmax_i),
            F_i = D_i/Dmax_i) %>%
...
Do not actually have to do "cumulative sum" here, they are already done. Just use the already generated A_il, alphia_il and beta_i.
