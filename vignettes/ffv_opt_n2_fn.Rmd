...
# st_title <- paste0('Optimal choices as resource expands at three inequality aversion (λ) Levels')
st_subtitle <- paste0('Red Dots = the maximum feasible expected outcomes grid (rectilinear)\n',
                      'Black Solid Lines = indifference curves for λ=-0.01\n', 
                      'Black Dashed Lines = discrete aggregate resource levels')
st_caption <- paste0('Expected Outcome Two People Example')
st_x_label <- expression(EH[1]~'('~D[1]~'): '~Expected~Outcome~Allocation~Recipient~1)
st_y_label <- expression(EH[2]~'('~D[2]~'): '~Expected~Outcome~Allocation~Recipient~2)
plt_disc_paper <- plt_disc_paper +   
  labs(subtitle = st_subtitle, 
       x = st_x_label, 
       y = st_y_label, 
       caption = st_caption) +
...
st_subtitle <- paste0('Rectangular bounds on choices\n',
                      'Black Solid Lines = indifference curves for λ=-0.01\n', 
