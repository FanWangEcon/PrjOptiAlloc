...
The key concept that make the solutions possible is the *Optimal Allocation Queue*. It turns out, in both discrete and continuous cases, we can analytical solve for the optimal ranking in which individuals should begin and stop to receive allocations. These rankings, crucially, are invariant to resources when the marginal effects of allocations are nonincreasing. Nonincreasing marginal effects lead to resource (income) expansion paths do not bend backwards.
