...
for (it_rho_ctr in seq(1,length(ar_rho))) {
  ...
  # l. optimal allocation individual key + optimal allocation
  tb_opti_main_results <- tb_opti %>%
    select(-one_of(c('lowest_rank_alpha', 'lowest_rank_beta')))
  tb_opti_allocate_wth_key <- tb_opti %>% select(one_of('indi.id','opti_allocate')) %>%
                                rename(!!paste0('rho_c', it_rho_ctr) := !!sym('opti_allocate'))
  
  # m. merge optimal allocaiton results from different planner preference
...
# o. graph scatter of opposing preference correlation
svr_lowrho_ctr <- 1
svr_higrho_ctr <- length(ar_rho)-1
svr_lowrho <- paste0('rho_c', svr_lowrho_ctr)
svr_higrho <- paste0('rho_c', svr_higrho_ctr)
