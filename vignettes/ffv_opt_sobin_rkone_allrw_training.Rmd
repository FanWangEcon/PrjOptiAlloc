...
title_line2 <- sprintf("Compare observed (random) allocations to optimal allocations")
title_line3 <- sprintf("Expenditure (Resource) Minimization to achieve same welfare as welfare at observed allocations")

# Relabel Variable
Outcome_levels <- c(rev = "employment_prob")
tb_rev_wage_employ_long <- tb_rev_wage_employ_long %>% 
  filter(Outcome == 'employment_prob') %>%
  mutate(Outcome = as_factor(Outcome)) %>%
  mutate(Outcome = fct_recode(Outcome_levels, !!!Outcome_levels))
