...
# combine results
mt_combine <- cbind(ar_rho, ar_rev_wage*100, ar_rev_employ*100)
colnames(mt_combine) <- c('lambda', 'rev_expected_wage', 'rev_employment_prob')
tb_rev_wage_employ <- as_tibble(mt_combine) %>% rowid_to_column(var = "eval")
# Transform x-scale to 1-lambda
tb_rev_wage_employ <- tb_rev_wage_employ %>% mutate(one_minus_lambda = 1 - lambda)
...
# title line 2
title_line1 <- sprintf("Percentage of Training Spots Misallocated, NSW Lalonda (AER, 1986)")
title_line2 <- sprintf("REV (Resource Equivalent Variation) Along Planner Spectrum")

# Graph Results--Draw
line.plot <- tb_rev_wage_employ_long %>% 
  ggplot(aes(x=one_minus_lambda, y=rev,
             group=Outcome, 
  ...
  labs(title = paste0(title_line1, '\n', title_line2),
       ...
       y = 'Misallocation Percentage = REV',
       caption = 'Training RCT with 297 treated, 425 untreated. Reallocate 297 training spots.') +
...
dfj_hist %>% ggplot(aes(x=A_i_wage, fill=alpha_i_wage)) +
      ...
      labs(fill="") %>%
      labs(title = paste0('Wage A_i Distribution by alpha_i (two unique levels, Age <= 23 and > 23)'),
             x = 'A_i = Expected Wage no Training',
             y = 'density',
